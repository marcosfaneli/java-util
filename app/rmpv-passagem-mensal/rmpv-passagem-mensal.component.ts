...
@Component({
  selector: 'app-rmpv-passagem-mensal',
  templateUrl: './rmpv-passagem-mensal.component.html',
  styleUrls: ['./rmpv-passagem-mensal.component.css']
})
export class RmpvPassagemMensalComponent extends ListaBase implements OnInit {
  ...
  constructor(service: RmpvPassagemMensalService, toast: ToastrService, router: Router, private route: ActivatedRoute) {
  ...
  totalizarMarcas(marca: any, total: any) {
    marca.dados.forEach(element => {
      ...
      total.dados[index].total += element.total;
    });
  }
  ...
  somarTotalGeralPorMarca(marca: any) {
    ...
    item.dados.forEach(element => {
      ...
      const fat = marca.dados[posicao].total;
      element.total += fat;
    });
  }
  ...
  totalizarCategorias(marca: any) {
    ...
    marca.dados.forEach(categoria => {
      ...
      internas += categoria.interna ? categoria.total : 0;
    });
  ...
  public definirFiltragem() {
    $('#rmpv-passagem-mensal-filtro').modal();
  }

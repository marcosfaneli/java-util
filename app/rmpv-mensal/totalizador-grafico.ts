export class TotalizadorGrafico {
    ...
    private criarCategoria(dados: any[], marca: number) {
        dados.forEach(item => {
            ...
            categoria.valor_jaguar += marca === 40 ? item.valorMaoObraUnitario : 0;
            categoria.valor_land_rover += marca === 31 ? item.valorMaoObraUnitario : 0;
        });
    }
    ...
    public getDadosLandRover(): any[] {
        const dados: any[] = []; 
        dados.push(this.valoresJaguarVazio());    
        dados.push(this.valoresLandRover());   
       
        return dados;
    }

    public getDadosVazio(): any[] {
        const dados: any[] = []; 
        dados.push(this.valoresJaguarVazio());    
        dados.push(this.valoresLandRoverVazio());   
